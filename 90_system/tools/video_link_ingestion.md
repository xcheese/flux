...
- 知识库展示层只展示分析报告、沉淀笔记和可读结论。
- 视频素材目录中的 `metadata.json`、`audio.*`、`video.*`、`transcript.*`、`ocr.*`、`frames/` 属于证据数据，不进入文档列表。
- 若采用 `10_raw/videos/<slug>/` 目录形态，展示入口固定为 `analysis.md`；其余文件仅供复核和重分析使用。
- 分类标签应优先描述文档类型和主题，例如 `视频分析`、`AI情报`、`概念`、`AI工具`、`工作流`；不要把“事实/观点/推断”这类报告字段误当成文章归类。
